...
parser = argparse.ArgumentParser(description='Lunes Asset Rich List')
...
try:
    issuetx = requests.get( node + '/transactions/info/%s' % assetId).json()
except:
    print("Asset not found!")
if issuetx['id'] == assetId:
    assetName = str(issuetx['name'])
    assetDec = issuetx['decimals']
    states = requests.get( node + '/debug/state').json()
    print("-" * 64)
