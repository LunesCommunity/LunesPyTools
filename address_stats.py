...
def blocks_reader(seq_from, seq_to, index):
    thread_blocks[index] = requests.get('%s/blocks/seq/%d/%d' % (node, seq_from, seq_to)).json()

parser = argparse.ArgumentParser(description='Statistics for Lunes address')
parser.add_argument('address', type=str, help='lunes address')
parser.add_argument('-b', '--blocks', action='store_true', help='show the list of blocks mined by the address')

args = parser.parse_args()
address = args.address

valid = requests.get(node + '/addresses/validate/' + address).json()['valid']
if valid == False:
...
try:
    address_balance = float(requests.get(node + '/addresses/balance/' + address).json()['balance']) / 100000000
except:
    address_balance = 0
print("-" * 90)
print
...
assets = requests.get(node + '/assets/balance/' + address).json()['balances']
for asset in assets:
